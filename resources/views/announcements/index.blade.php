...
@section('title', 'Unread Announcements')

@section('content')

    <div class="bg-gray-200 min-h-screen">
        <div class="max-w-3xl mx-auto pt-8">
            <h1 class="text-2xl font-medium mb-2 text-indigo-500">Announcements</h1>
            @foreach($announcements as $announcement)
                <div class="max-w-3xl mx-auto mt-8">
                    <div class="rounded-lg w-full px-8 py-6 bg-white shadow cursor-pointer border-transparent border-2 hover:border-indigo-500 clearfix" onclick="window.location = '/announcement/{{ $announcement->id }}'">
                        ...
                        <a href="/announcement/{{ $announcement->id }}" class="underline float-right text-xs text-blue-600">Read More</a>
                    </div>
                </div>
            @endforeach
            @if(count($announcements) < 1)
                <div class="rounded-lg w-full px-8 py-6 bg-white mt-4">
                    <p class="mt-2 text-gray-600 text-base">You have no unread announcements.</p>
